...
class ApiTests(TestCase):
    ...
    def test_idle_face_discarded_when_content_present(self) -> None:
        controller = DisplayController(
            FakeEpd(),
            DisplayState(
                face="look_center",
        ...
        try:
            ...
            request = urllib.request.Request(
                f"http://127.0.0.1:{controller.state.port}/message",
                data=json.dumps({"face": "look_left", "content": "hello"}).encode("utf-8"),
                headers={"Content-Type": "application/json"},
                method="POST",
            )
            with urllib.request.urlopen(request) as response:
    ...
    def test_idle_face_without_content_returns_status(self) -> None:
        controller = DisplayController(
            FakeEpd(),
            DisplayState(
                face="look_center",
        ...
        try:
            ...
            request = urllib.request.Request(
                f"http://127.0.0.1:{controller.state.port}/message",
                data=json.dumps({"face": "look_left"}).encode("utf-8"),
                headers={"Content-Type": "application/json"},
                method="POST",
            )
            with urllib.request.urlopen(request) as response:
    ...
    def test_mcp_send_to_inkypal_tool_reports_idle_face_without_update(self) -> None:
        controller = DisplayController(
            FakeEpd(),
            DisplayState(
                face="look_center",
