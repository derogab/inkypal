...
| `OPENAI_OPENROUTER_REFERER` | OpenRouter attribution URL override (optional). Used as `HTTP-Referer` when `OPENAI_BASE_URL` points to OpenRouter. | https://github.com/derogab/inkypal |
| `OPENAI_OPENROUTER_TITLE` | OpenRouter attribution title override (optional). Used as `X-OpenRouter-Title` when `OPENAI_BASE_URL` points to OpenRouter. | InkyPal AI |
| `OPENAI_OPENROUTER_CATEGORIES` | Comma-separated OpenRouter attribution categories (optional). Used as `X-OpenRouter-Categories` when `OPENAI_BASE_URL` points to OpenRouter. | - |

When `OPENAI_BASE_URL` points to OpenRouter, `InkyPal` sends OpenRouter attribution headers by default using the repository URL and `InkyPal AI`. Set the `OPENAI_OPENROUTER_*` variables to override them.
...
# OPENAI_MODEL=auto
# OPENAI_OPENROUTER_REFERER=https://github.com/derogab/inkypal
# OPENAI_OPENROUTER_TITLE=InkyPal AI
# OPENAI_OPENROUTER_CATEGORIES=personal-agent
EOF
```
