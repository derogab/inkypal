...
InkyPal also exposes a remote Streamable HTTP MCP server at `/mcp`.
...
Replace `<key>` with the same value used for `INKYPAL_API_KEY`. If `INKYPAL_API_KEY` is unset, omit the `headers` block.
